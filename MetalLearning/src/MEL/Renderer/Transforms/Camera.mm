...
namespace MEL {
	...
	void Camera::SetOrthoZoom(float zoomLevel){
		float height=10.0f/zoomLevel;
		float width=height*m_AspectRatio;
		
		m_OrthoTop=height*0.5f;
		m_OrthoBottom=-height*0.5f;
		m_OrthoRight=width*0.5f;
		m_OrthoLeft=-width*0.5f;
	...
	void Camera::UpdateProjectionMatrix(){
		if(m_IsPerspective){
			...
			float basedHeight=m_OrthoTop-m_OrthoBottom;
			float adjustedWidth=basedHeight*m_AspectRatio;
			
			float left=-adjustedWidth*0.5f;
			float right=adjustedWidth*0.5f;
			...
			m_ProjectionMatrix=(simd::float4x4){
				...
				(simd::float4){-(right+left)/(right-left),-(top+bottom)/(top-bottom),
